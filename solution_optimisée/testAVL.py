...
from avlmerge.avlmerge import MergeAVL

class Solution:
    def __init__(self, A1: TreeAVL, A2: TreeAVL, order1: str, order2: str):
        self.A1 = A1
        self.A2 = A2
        self.merge = MergeAVL(A1, A2)
        self.order1 = order1
        self.order2 = order2
    
    def merge_trees(self)-> TreeAVL:
        return self.merge.merge(self.order1, self.order2)
    
    def merge_n_trees(self, trees: List[TreeAVL])-> TreeAVL:
    ...
    def run(self):
        ...
        print("A4:")
        A4.print_tree()
        A5: TreeAVL = self.merge_n_trees([self.A1, self.A2, A4])
